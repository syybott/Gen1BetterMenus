...
local function renderWideEnemy(battle, fx)
  ...
  drawStatus(battle, battler, hasGender and 80 or 88, 8)
  if hasGender then
    drawBattleGender(battle, battler, 80, 8)
  end
  ...
  -- Protect only the native bar's two fill rows from the palette shader,
  -- matching how the XP meter works below. Exempting the entire row or label
  -- causes fractional scissor clamping at the scaled composite to bleed a
  -- 1-output-pixel white seam to the left of the 'H'.
  ...
  PaletteFX.markTrueColor(24 + hudShake, 19, 88, 2)
...
local function renderWidePlayer(battle)
  ...
  drawStatus(battle, battler, hasGender and 256 or 264, 64)
  if hasGender then
    drawBattleGender(battle, battler, 256, 64)
  end
  ...
  PaletteFX.markTrueColor(208, 75, 88, 2)
